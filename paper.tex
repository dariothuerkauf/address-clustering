...
Our implementation of node embedding methods differs from previous approaches by \cite{Beres2020} and \cite{wu2022tutela}. We construct the network graph based on asset transfers between addresses of our predefined set and analyze on-chain data from two distinct blockchain networks: Polygon and Ethereum. 
...
Subsequently, we applied three node embedding algorithms that showed strong performance in the experiments of Beres et al. and compared their effectiveness. Our results indicate that Role2Vec is the best-performing method, locating the ground truth pair within the five nearest neighbors in 70\% of cases. This contrasts with Beres's findings, which identified Diff2Vec as the best method. \newline
However, determining why one approach outperforms another is challenging. Both Diff2Vec and DeepWalk preserve neighborhood characteristics, while Role2Vec is a structural node embedding method. % expand on structural node embedding and give possible explanation
...
Wir haben eine eigene Methode entwickelt zum Clustern von Network embeddings.
Wir bauen Clusters abhängig von Euclidischer distanz zwischen feature vectors von nodes. 
Dazu benutzen wir die embeddings von role2vec.
