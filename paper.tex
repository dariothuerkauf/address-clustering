...
In this thesis, we set out to cluster 473,927 addresses observed in Decentraland. Focusing on on-chain data on Ethereum and Polygon, we reduced the set to 137,544 addresses. Depending on various assumptions, we were able to cluster entities interacting through more than one address. Merging the clusters after applying each of the three clustering heuristics and assuming a treshold value of 0.5 for node embedding, we could cluster X addresses to Y entities. Further assuming that all other addresses are single clusters, we argue that the total number of active users during that time frame was Z.
