...
Conversely, \cite{wu2022tutela} aim to cluster addresses without a predefined set, therefore using transaction data for network graph construction. \cite{Beres2020} try to match deposit and withdrawal addresses for Tornado Cash, relying on all token transfers and transactions to cluster addresses based on similar on-chain behavior. Since it is not very common that Tornado cash withdrawal and deposit addresses interact with each other, the approach chosen by \cite{Beres2020} is better suited for their objectives. 

Moreover, we validated our approach and found better results in terms of average rank of ENS pairs, when building the network graph out of intra-set asset transfers compared to using all token transfer events and/or transactions. \newline
...
We applied three node embedding algorithms that showed strong performance in the experiments by \cite{Beres2020} and assessed their effectiveness. Our results indicate that Role2Vec is the best-performing method, locating the ground truth pair within the five nearest neighbors in 70\% of cases.  This diverges from previous findings that favored Diff2Vec as the most effective of the three methods. \newline
Determining why one approach outperforms another is complex. Diff2Vec and DeepWalk, both neighborhood-preserving algorithms, mainly capture node proximity. However, as noted by ..., this does not necessarily imply similarity, especially when addresses may frequently interact without being controlled by the same entity. In contrast, Role2Vec, a structural node embedding method, identifies nodes with similar network roles. In our scenario, network position or role may better represent similarity and structural relatedness among nodes in our evaluation sample. Despite these insights, the differences in results could be specific to the data at hand, given the small sample size, potential evaluation metric flaws, and the overall robust performance of all methods. \newline
