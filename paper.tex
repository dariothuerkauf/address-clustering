...
We gathered all transactions and token transfers up until block height \texttt{17,670,000} on Ethereum (July 11, 2023) and block \texttt{44,990,000} on Polygon (July 12, 2023). The output was saved in separate CSV files. We then imported this data into a MongoDB database\footnote{\url{https://www.mongodb.com/}}.
In total, we collected 30,689,978 token transfers and 16,092,531 normal transactions. Figure X visualizes the number of daily transactions and token transfers for each chain. Can see activity.

Transfer Events: 30,689,978
Transfer Events Ethereum: 7,832,778
Transfer Events Polygon: 22,857,200

Statistics about data, how many transactions on Polygon, how many on Ethereum, how many transfers on Polygon, how many on Ethereum -> Show in Figure
...
Transaction hash, timestamp in seconds from the UNIX epoch, gas used in the transaction, specified gas price, nonce




The transaction data fields contain the transaction hash, from and to address, the timestamp in seconds from the UNIX epoch, a nonce, value, input (often referred to as calldata), the amount of gas used in the transaction, the specified gas price, the name of the function called (added by Etherscan) and the name of the blockchain.
