...
To find self-authorization, we analyze the transaction data of the addresses, focusing on the transaction's ``calldata``. The first four bytes of the calldata represent the selector of the function signature, which specifies the smart contract function to be invoked. \newline
...
For any node within the embedding, we can easily order all other nodes (addresses) by the Euclidean distance. %We can easily order nodes by the Euclidean distance to a given node. 
We achieve this through the Faiss library, suitable for efficient similarity search and clustering of dense vectors.  \newline
...
Role2Vec seems to be the most effective method, with the target address, on average, being found within the 28 nearest neighbors. In contrast, Diff2Vec and DeepWalk produced slightly higher averages of 46 and 62, respectively.
Furthermore, Role2Vec found the target address within the five nearest neighbors in 19 of 34 cases and within the ten nearest neighbors in 71\% of cases.
...
%We can say that for these address pairs, the methods are all surprisingly good in finding the target address -> node embedding methoden schreiben den beiden addressen/nodes sehr ähnliche feature vectoren zu.
...
%Since we have Random guess would yield an average of 25k. Maximum is 51,566. 
...
After evaluating the different node embedding methods, we want to cluster addresses to assess the number of entities represented by the 51,566 nodes in the network/addresses. %to make a statement about the real number of users 
One approach might involve assigning the $k$ nearest neighbours of a node to a cluster and subsequently merging clusters until each node is assigned to exactly one cluster. However, selecting an appropriate $k$ is challenging, and it is hard to make the case that each address should belong to a cluster of $k$/ that each entity uses at least $k$ addresses.
Instead, we choose to build clusters based on a Euclidean distance treshold value. All neighbors of a node that fall below this treshold are considered a cluster. Subsequently, we merge overlapping clusters; for instance, if node A is clustered with nodes B and C, and node C is clustered with X, we form a cluster comprising A, B, C, and X.
We explored various treshold values and visualized the resulting cluster sizes in a histogram. %to better understand the distribution
