...
Combining the results of self-authorization, learned node embedding and LAND transfers, we can cluster 37.5\% (51,581 out of 137,544) of addresses representing 29,680 entities. \newline
We assume that the unclustered addresses each represent individual entities, a (strict) distance threshold of 0.5 for clustering addresses generated by Role2Vec and remove the 71-address cluster found in self-authorization.
We estimate a total of 115,643 active users during the studied time frame. Among them, 8,483 entities (X \%) interacted with Decentraland through more than one address. The largest entity consists of 499 addresses. The distribution of cluster sizes can be seen in Figure \ref{fig:barplot_clusters}.
...
The goal of this research/thesis was to identify the number of entities represented by the 473,927 addresses observed in Decentraland between. By focusing on Ethereum and Polygon on-chain data, we narrowed down the address set to 137,544. Combining the three clustering heuristics applied, we successfully clustered 37.5\% (51,581) of these addresses into 29,680 entities. \newline
Assuming the remaining addresses each represent individual entities, we estimate a total of 115,643 active users during the studied time frame. Among them, 8,483 entities used more than one address, with the largest entity having 499 addresses. The distribution of cluster sizes can be seen in Figure \ref{fig:barplot_clusters}.

Assumptions: Assumptions: Role2Vec with a distance threshold of 0.5 (strict)
Removed large self-authorization cluster 

Most clusters through graph representation learning
Very dependent on threshold
X \% use more than one address
Large address clusters could be some bot behavior
