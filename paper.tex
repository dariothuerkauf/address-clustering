...
The main intuition/assumption behind this approach is that users may want to reorganize their accounts from time to time, mostly to increase security guarantees or spread the risk around multiple accounts. As a concrete example, some users may have started with a simple browser-based wallet but as the value of their assets and their knowledge grew, they changed to a more secure hardware wallet. We detect 161 entities with 413 addresses applying this approach. One entity with 70 address is very likely a false-positive, including an address that participated in the intial LAND auction and subsequently distributed LAND. This may have happened through some off-chain compensation. The advantage of this method is that it is relatively easy to visualise and understand. The disadvantage is that it demands a specific on-chain behaviour and is only scalable with significant effort. Additionally, many events that occur off-chain are not detectable in the data, resulting in an increased occurrence of false positives. 
...
%740 transfers, 413 addressen und 161 or 160 entities with 342 addresses. -The large cluster may incorrectly be marked/maybe off-chain compensation? -> auction address
%%%%%%%%%%%%%%%%%%%%%

In this thesis, we set out to cluster 470k addresses observed in Decentraland. Focusing on on-chain data on Ethereum and Polygon, we reduced the address set to 135k. Depending on various assumptions, we were able to cluster entities interacting through more than one address. Merging the clusters resulting after applying each of the three clustering heuristics and assuming a treshold value of 0.5 for node embedding, we could cluster X addresses to Y entities. Assuming that all other addresses are single clusters, we argue that the total number of active users during that time frame was ...

other Limitations -> ENS pairs? Incorporate time-of-day/normalized gas into feature vectors, two step approach
Limitations: various assumptions, possibly false-positives, some clusters may well be undetected. 
