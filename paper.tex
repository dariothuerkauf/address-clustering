...
Address clustering through node embedding methods proved to be the most effective approach in our analysis. Unlike methods that require participation in specific on-chain events, node embedding examines the entire interaction network between addresses, making it broadly applicable. \newline
Our implementation of node embedding methods differs from previous approaches by \cite{Beres2020} and \cite{wu2022tutela}. We construct the network graph based on asset transfers between addresses of our predefined set and simultaneously analyze data from Polygon and Ethereum. The main reason is previous implementations' different settings and research questions. \cite{wu2022tutela} aim to cluster addresses without a predefined set, therefore using transaction data for network graph construction. Conversely, \cite{Beres2020} try to match deposit and withdrawal addresses for Tornado Cash, relying on all token transfers and transactions to cluster addresses based on similar on-chain behavior. Since it is not very common that Tornado cash withdrawal and deposit addresses interact with each other, the approach chosen by \cite{Beres2020} is better suited for their objectives. \newline
