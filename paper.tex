...
To find self-authorizations, we analyze the transaction data of the addresses, focusing on the transaction's ``calldata``. The first four bytes of the calldata represent the selector of the function signature, which specifies the smart contract function to be invoked. \newline
\textit{First}, we query all transactions that begin with a function signature related to the approval functions corresponding to ERC-20, ERC-721 and ERC-1155 token standards. This step helps us identify all approval transactions. \textit{Second}, we extract the approved address (spender address), which is always located at the same position within the calldata due to token standardization. We then record the spender address in a new field within our dataframe. \textit{Third}, we implement two filters on the approval transactions. The first filter checks if the spender address is part of our address set, and the second ensures that the spender address is different from the address initiating the transaction, which may occur due to a user interface bug or testing transactions. \textit{Fourth}, we group the remaining approvals by the initiating address and proceed to form the clusters.
...
For any node within the embedding, we can easily order all other nodes by the Euclidean distance. %We can easily order nodes by the Euclidean distance to a given node. 
We achieve this through the Faiss library, which is suitable for efficient similarity search and clustering of dense vectors. \newline
...
All three methods demonstrated remarkable efficacy in locating the target address, which indicates their ability to find multiple addresses controlled by a single entity. %Ability to assign highly similar feature vectors to both addresses within a ground truth pair.
Role2Vec seems to be the most effective method, with the target address, on average, being found within the 28 nearest neighbors. In contrast, Diff2Vec and DeepWalk produced slightly higher averages of 46 and 62, respectively. Furthermore, Role2Vec found the target address within the ten nearest neighbors in 24 out of 34 cases (71\%). Diff2Vec is slightly less effective than Role2Vec or DeepWalk, although still generally successful. \newline
It is noteworthy that two address pairs are not included in the network graph and therefore display no values. However, they have been included to demonstrate that the intra-set asset transfer approach may not always be applicable to cluster all addresses of a predefined set. \newline
We also need to consider, that average rank may not be the most appropriate evaluation metric, particularly when a user controls significantly than two addresses. For example, if a single entity controls 20 addresses. Only one pair of addresses is detected as ground truth. Even if the algorithm finds all 20 addresses within the nearest neighbors but identifies our ground truth pair as the last one, we will score it lower than another algorithm that finds only one of the twenty addresses but within, for example, the ten nearest neighbors.

%Role2Vec seems to work best out of the three, it has the lowest mean (28) and standard deviation and almost the same median as deepwalk.
%In contrast to Beres, where diff2vec worked best, in our case it is slightly less good then the other methods but still good overall.
...
After evaluating the different node embedding methods, we want to cluster addresses to assess the number of entities represented by the 51,566 nodes in the network. %to make a statement about the real number of users
We use the node embedding generated by Role2Vec.
One approach might involve assigning the $k$ nearest neighbours of a node to a cluster and subsequently merging clusters until each node is assigned to exactly one cluster. However, selecting an appropriate $k$ is challenging, and it is hard to make the case that each address belongs to a cluster of at least $k$ for entity identification. \newline
Instead, we choose to build clusters based on the Euclidean distance. All neighbors of a node below a certain treshold are considered a cluster (entity). Subsequently, we merge overlapping clusters; for instance, if node A is clustered with nodes B and C, and node C is clustered with X, we form a cluster comprising A, B, C, and X. \newline
We explore the senstitivity of clusters to various treshold values. The impact of changing the threshold value is shown in Figure \ref{fig:threshold_sensitivity} and Table \ref{tbl:cluster_table}. We chose these tresholds after manual inspection of ground truth pair distances. For better visibility, we only display clusters smaller than 20. At thresholds of 0.6, 0.7, and 0.8, we find 23,608, 18,592, and 14,793 clusters, respectively. Regardless of the threshold, the largest cluster always consists of more than 500 addresses. Unsurprisingly, higher tresholds result in fewer single-address clusters. The effect of the threshold is most pronounced for single address clusters, while affecting other cluster sizes much less. 
...
\scriptsize
  ...
  \begin{tabular}{lrrr}
    \hline
	\textbf{Threshold} & \textbf{0.6} & \textbf{0.7} & \textbf{0.8} \\
	\hline
Total clusters & 23,608 & 18,592 & 14,793 \\
Size of largest cluster & 516 & 532 & 544\\
Clusters of size >50 & 3 & 9 & 14\\
    \hline
  \end{tabular}
  \caption{A table}
...
For the LAND transfer heuristic, we started by selecting all LAND transfer events from our intra-set token transfer dataframe. We store the transaction hash as a list and later collect all transactions and token transfers for transactions within the hash list. Afterwards we delete duplicates. We detect purchases by .... If a transaction consisted of a purchase, we mark it in a new column. Later on, we filter for all simple transfers and build clusters.

For visualization we ....


\iffalse
Intra-token-transfer events -> select all LAND transfers -> report the transaction hash\\
find all transfer events and transactions for the list of transaction hashes, drop duplicates, combine transactions and token transfers\\
Detect purchases, report in new column\\
Filter for simple transfers, build clusters\\
\fi
...
%Discussion is an in-depth exploration of the results, going into detail about the meaning of your findings and citing relevant sources to put them in context.
